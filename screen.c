...
void screen_draw_layout() {
  screen_draw_box(0,0,VIDEO_FILS,VIDEO_COLS, ' ', 0X00);
  uint32_t fila = VIDEO_FILS/2;
  uint32_t columna = VIDEO_COLS/4;
  uint8_t colorBackground = C_BG_BLUE;
  uint8_t colorFarground = C_FG_MAGENTA;
  uint8_t atributo = (colorBackground) | (colorFarground & 0x0F);
  
  char mensaje[] = "ALBARO GONZALES | MATEO RUIZ | JUANI BUSTOS";
  
  screen_draw_box(fila-1,columna-1,3,43+2, ' ', colorBackground);

  print(mensaje, columna, fila, atributo);
